 ...
 * missing detail, it is not a layer at all. The empty string is a name, and a poor one, but
 * `createPlugin` accepts it and writes it here, so rejecting it would make this guard throw
 * away the factory's own output and lint against the defaults instead. Judging the names is
 * not this guard's job; reading the shapes the factory can write is.
 */
 ...
 *
 * A list carrying an entry that can name no layer falls back to the defaults rather than
 * throwing, all of it and not just that entry, because a rule has no way to reject
 * configuration except by throwing, and throwing is the failure this guard exists to remove.
 * `segments` is read the same way three lines away.
 */
