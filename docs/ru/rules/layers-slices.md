...
Проверяет импорты между слоями и слайсами согласно методологии Feature-Sliced Design.

**Имя правила:** `@conarti/feature-sliced/layers-slices`

## Описание правила

Это правило обеспечивает соблюдение двух основных принципов FSD:

1. **Иерархия слоёв** — нижние слои не могут импортировать из верхних
2. **Изоляция слайсов** — слайсы одного слоя не могут импортировать друг из друга
...
::: warning
Работает только с явным ключевым словом `type`. Обычные импорты типов всё равно проверяются.
:::

```ts
// Всё ещё ошибка, даже если User — только тип
import { User } from 'entities/user';
```
...
- При миграции на FSD — лучше использовать `ignorePatterns`
- В тестовых файлах с кросс-слойными импортами — используйте `ignoreInFilesPatterns`
